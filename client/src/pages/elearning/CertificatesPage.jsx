...
// Lazy import heavy PDF deps only when user clicks download
let cachedPDFLib = null;
async function downloadCertPDF(cert, user) {
  if (!cachedPDFLib) {
    const [{ pdf }, { default: CertificatePDF }] = await Promise.all([
      import('@react-pdf/renderer'),
      import('@/components/elearning/domain/CertificatePDF'),
    ]);
    cachedPDFLib = { pdf, CertificatePDF };
  }
  const { pdf, CertificatePDF } = cachedPDFLib;
  const blob = await pdf(
    <CertificatePDF
      certificate={cert}
      recipient={user}
      path={cert.learningPath || {}}
      instructor={{ username: 'Saenal Aladin Rapi' }}
    />
  ).toBlob();
  const url = URL.createObjectURL(blob);
  const a = document.createElement('a');
  a.href = url;
  a.download = `${cert.serialNumber || 'certificate'}.pdf`;
  ...
  return (
    <div className="space-y-6">
      ...
      {isLoading ? (
        ...
        <div className="grid gap-4 sm:grid-cols-2">
          {certs.map((c) => <CertCard key={c.id} cert={c} userName={user.username} user={user} />)}
        </div>
      )}
...
function CertCard({ cert, userName, user }) {
  const path = cert.learningPath || {};
  const handleDownload = async () => {
    try {
      await downloadCertPDF(cert, user);
    } catch (e) {
      console.error('Failed to generate PDF', e);
      alert('Gagal generate PDF — coba lagi atau hubungi admin');
    }
  };
  return (
      ...
      <div className="relative">
        ...
        <div className="mt-4 flex gap-2">
          <button
            ...
            className="inline-flex items-center gap-1.5 rounded-md bg-slate-900 px-3.5 py-2 text-[12.5px] font-semibold text-white hover:bg-orange-600 transition-colors"
          >
