...
export default function MinimalContact() {
  ...
  const onSubmit = async (e) => {
    ...
    try {
      ...
      showToast.error(e.response?.data?.message || t.contact.failed);
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <div className="bg-white">
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 py-12">
        <h1 className="text-2xl font-semibold text-gray-900">{t.contact.title}</h1>
        ...
        <div className="mt-8 grid grid-cols-1 md:grid-cols-2 gap-8">
          <form onSubmit={onSubmit} className="rounded-lg border border-gray-200 p-6 space-y-4">
            <div>
              <label className="block text-sm text-gray-700">{t.contact.name}</label>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div>
                <label className="block text-sm text-gray-700">{t.contact.email}</label>
              ...
              <div>
                <label className="block text-sm text-gray-700">{t.contact.phone}</label>
            ...
            <div>
              <label className="block text-sm text-gray-700">{t.contact.message}</label>
            ...
            <button disabled={loading} className="w-full md:w-auto rounded-md bg-gray-900 text-white px-5 py-2 text-sm hover:bg-black disabled:opacity-50">
              {loading ? t.sending : t.contact.send}
            </button>
          </form>
